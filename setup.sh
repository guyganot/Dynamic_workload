...
echo "Now we will test the endpoints:"
echo "enqueue work to instance 1:"
curl -X PUT "http://$PUBLIC_IP_1:5000/enqueue?iterations=1"
echo "----------------------------------------------------------"
echo "enqueue work to instance 2:"
curl -X PUT "http://$PUBLIC_IP_2:5000/enqueue?iterations=2"
echo "----------------------------------------------------------"
echo "Now the script will wait 10 minutes (600 sec). It allows time for the instances to be deployed and for the tasks to be processed"
sleep 600
echo "----------------------------------------------------------"
echo "pull completed tasks from instance 1"
